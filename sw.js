...
// baixas (1.03–1.20) ainda precisam permitir que os perfis altos sejam construídos
// acumulando várias seleções coerentes. Faltas/cartões do azarão podem reforçar a
// pressão ofensiva do favorito, então esse cenário não deve morrer no Conservador.
...
for (const key of ['valor','arriscado','muito-arriscado','jackpot']) {
  assert(ultraGroups[key].some(t => !t.unavailable), `${key} deve existir mesmo quando as odds fortes individuais são muito baixas.`);
}
assert(ultraGroups.valor.every(t => t.unavailable || (t.combinedOdd >= 2 && t.combinedOdd <= 4)));
assert(ultraGroups.arriscado.every(t => t.unavailable || (t.combinedOdd >= 4 && t.combinedOdd <= 6)));
assert(ultraGroups['muito-arriscado'].every(t => t.unavailable || (t.combinedOdd >= 6 && t.combinedOdd <= 8)));
assert(ultraGroups.jackpot.some(t => !t.unavailable && t.combinedOdd >= 8));
console.log('OK: poucos jogos com odds muito baixas ainda montam Valor/Arriscado/Muito Arriscado/Jackpot.');
