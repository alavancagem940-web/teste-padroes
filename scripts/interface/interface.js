...
const Interface = {
    iniciar(){
        ...
        app.innerHTML=`<div class="painel">
          <header><h1>Painel de Padrões Fictícios</h1><div id="relogio-partidas" class="relogio-partidas">🕐 Horário das partidas: --:-- · <b>Próximo jogo: --:--</b></div>
    ...
    atualizar(){
        ...
        if(typeof RelogioPartidas!=='undefined'){ const a=RelogioPartidas.agora(), n=RelogioPartidas.proximaPartida(); const el=document.getElementById('relogio-partidas'), ph=document.getElementById('proximo-horario'); if(el)el.innerHTML=`🕐 Londres: <b>${String(a.hour).padStart(2,'0')}:${String(a.minute).padStart(2,'0')}:${String(a.second).padStart(2,'0')}</b> · <b>Próximo jogo: ${n.horario}</b>`; if(ph)ph.textContent=n.horario; }
        ...
        document.getElementById('ultimo-registro').textContent=ultimoEvento?(ultimoEvento.tipo==='vazio'?'VAZIO':ultimoEvento.placar):'-';
        document.getElementById('sequencia-atual').textContent=seq.length;
        document.getElementById('ultimos-sequencia').innerHTML=eventosSessao.length?eventosSessao.map(x=>x.tipo==='vazio'?`<span class="registro-vazio" title="${x.horario||''} sem resultado">VAZIO</span>`:`<span title="${x.horario||''}">${x.placar}</span>`).join('<b class="seta">→</b>'):'<span class="muted">Nenhum resultado na sequência atual.</span>';
