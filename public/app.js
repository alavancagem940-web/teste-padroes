...
function uniquePlayerPreview(markets, teamName) {
  ...
  for (const m of markets || []) {
    ...
    if (seen.has(key)) continue; seen.add(key); rows.push(m);
  }
  const quotas = [['G',1],['D',4],['M',3],['F',3]];
  const selected = [], used = new Set();
  for (const [pos,limit] of quotas) {
    for (const row of rows.filter(x=>String(x.position||'').toUpperCase().startsWith(pos)).slice(0,limit)) { selected.push(row); used.add(builderMarketText(row.player)); }
  }
...
function renderPlayerLineupPreview(match, markets) {
  const teams = [match?.homeTeam, match?.awayTeam].filter(Boolean);
  return `<div class="player-lineup-preview">${teams.map(team=>{
    const players = uniquePlayerPreview(markets, team);
    return `<section class="player-lineup-team"><div class="player-lineup-team-head">${teamLogoHtml(match,team)}<div><strong>${safe(team)}</strong><small>prévia por posição • titulares só quando a fonte confirmar</small></div></div><div class="player-preview-list">${players.length?players.map(p=>`<span class="player-preview-chip">${playerAvatar(p,'player-preview-avatar')}<span>${safe(p.player)}</span><b>${safe(playerPositionLabel(p.position))}</b></span>`).join(''):'<span class="micro">Elenco ainda não carregado.</span>'}</div></section>`;
  }).join('')}</div>`;
}
function renderPlayerMarketBoard(matchIndex, indexedRows=[]) {
  ...
  if (!rows.length) {
    board.innerHTML = `<div class="player-market-empty ${playerMarketLoading.has(matchIndex)?'player-market-loading':''}">${safe(status || 'Carregando mercados de jogadores desta partida…')}</div>`;
    return;
  }
  const order = ['Chutes no gol do jogador','Chutes do jogador','Defesas do goleiro','Assistências do jogador','Chutes no Gol - Primeiro Tempo','Chutes - Primeiro Tempo','Passes do jogador','Desarmes do jogador','Impedimentos do jogador','Faltas do jogador','Faltas sofridas','Cartão do jogador','Gol do jogador','Primeiro marcador'];
  const families = [...new Set(rows.map(x=>x.market))].sort((a,b)=>{const ai=order.indexOf(a),bi=order.indexOf(b);return (ai<0?999:ai)-(bi<0?999:bi)||a.localeCompare(b);});
  const meta = playerMarketMetaByMatch[matchIndex] || {};
  board.innerHTML = `<section class="player-market-intro"><div class="player-market-intro-head"><div><span class="eyebrow">JOGADORES</span><h4>${safe(match.homeTeam)} x ${safe(match.awayTeam)}</h4></div><span class="player-market-count">${rows.length} opções</span></div><div class="micro">${safe(meta.note || 'Linhas organizadas por atleta. Quando não existe odd real, o Bilhete Plus mostra a odd justa estimada pelo modelo.')}</div>${renderPlayerLineupPreview(match,rows)}</section><div class="player-market-sections">${families.map((family,fi)=>{
    const familyRows = rows.filter(x=>x.market===family);
    const teamNames = [...new Set(familyRows.map(x=>x.team).filter(Boolean))];
    return `<details class="player-market-section" ${fi<3?'open':''}><summary><strong>${safe(playerMarketTitle(family))}</strong><span>${familyRows.length} linhas</span></summary><div class="player-market-section-body">${teamNames.map(team=>{
      const teamRows=familyRows.filter(x=>x.team===team); const byPlayer=new Map();
      for(const row of teamRows){const key=builderMarketText(row.player||row.selection);if(!byPlayer.has(key))byPlayer.set(key,[]);byPlayer.get(key).push(row);}
      return `<section class="player-team-block"><div class="player-team-title">${teamLogoHtml(match,team)}<strong>${safe(team)}</strong></div>${[...byPlayer.values()].map(options=>{
        const base=options[0];
        return `<div class="player-odds-row"><div class="player-identity">${playerAvatar(base)}<div class="player-identity-text"><strong>${safe(base.player||base.selection)}</strong><small>${safe(playerPositionLabel(base.position))}${base.expectedMinutes?` • ~${safe(base.expectedMinutes)} min`:''}</small></div></div><div class="player-odd-options">${options.sort((a,b)=>Number(a.target||0)-Number(b.target||0)).map(m=>{
          const signature=`${matchIndex}|${m.key}|${m.selection}`; const selected=customTicket.legs.some(l=>l.signature===signature); const real=m.oddType==='real'&&m.odd; const price=real?m.odd:(m.displayOdd||m.fairOdd||1/Math.max(.001,m.probability));
          return `<button type="button" class="player-odd-btn${selected?' is-selected':''}" data-player-market-index="${m._allIndex}"><span class="player-odd-line">${safe(playerLineLabel(m))}</span><strong class="player-odd-price">${money(price)}</strong><small class="player-odd-kind">${real?'real':'justa'}</small></button>`;
        }).join('')}</div></div>`;
      }).join('')}</section>`;
    }).join('')}</div></details>`;
  }).join('')}</div>`;
  board.querySelectorAll('.player-odd-btn').forEach(btn=>btn.addEventListener('click',()=>{
