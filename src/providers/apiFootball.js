...
function normalizeSeasonPlayers(body, teamId, leagueId) {
  ...
  for (const row of body?.response || []) {
    ...
    // A consulta é feita por time + temporada (sem travar por competição), porque em
    // jogos de copa a API pode não devolver o atleta quando filtramos também pela liga.
    // Ainda preferimos a liga principal solicitada; se ela não estiver no retorno,
    // usamos a competição em que o jogador acumulou mais minutos na temporada.
    const stats = allStats.find(s => Number(s.league?.id) === Number(leagueId)) ||
      [...allStats].sort((a,b) => num(b.games?.minutes) - num(a.games?.minutes))[0];
    if (!stats) continue;
    ...
    const assists = num(stats.goals?.assists);
    const passes = num(stats.passes?.total);
    const tackles = num(stats.tackles?.total);
    const offsides = num(stats.offsides ?? stats.games?.offsides);
    rows.push({
      id: row.player?.id,
      name: row.player?.name || '',
      photo: row.player?.photo || '',
      jersey: stats.games?.number || '',
      ...
      tacklesPer90: per90(tackles, minutes),
      offsidesPer90: per90(offsides, minutes)
    });
  }
  return rows;
}
...
function normalizeSquadPlayers(body, teamId) {
  const group = (body?.response || [])[0] || {};
  ...
  return (group.players || []).map(player => ({
    ...
    // O elenco é usado como caminho leve. Estes campos mínimos ativam o modelo
    // conservador por posição sem fingir que temos estatística de temporada.
    appearances: 1,
    starts: 0,
    minutes: 60,
    shotsPer90: 0,
...
export async function getTeamSquad(teamId) {
  const seconds = Number(process.env.PLAYER_SQUAD_CACHE_SECONDS || 43200);
  return cached(`api-football:squad:${teamId}`, seconds * 1000, async () => {
    const body = await request('/players/squads', { team: teamId });
    return normalizeSquadPlayers(body, teamId);
  });
}
...
export async function getTeamPlayerSeasonStats(teamId, leagueId, season) {
  ...
  return cached(`api-football:player-season:${teamId}:${leagueId}:${season}:team-season`, seconds * 1000, async () => {
    // Uma página por time para preservar a cota gratuita. Importante: não enviamos
    // `league` aqui. A própria documentação aceita team + season e isso evita perder
    // jogadores quando a partida do dia é de copa/supercopa ou quando a competição
    // retornada para o fixture não é a principal do clube.
    const body = await request('/players', { team: teamId, season, page: 1 });
