...
import { normalizeEspnEvent, normalizeSportsDbEvent, getPublicFixtures, getPublicFavoriteFixtures, getPublicSnapshots, findPublicFixtureByTeams, getPublicPlayerSquads } from './src/providers/publicFixtures.js';
...
for (const category of ["resultado","mais-menos","gols","escanteios","cartoes","chutes","faltas","defesas","jogadores","outros"]) {
...
assert(swSource.includes('v5-7-3-jogadores-publicos') && swSource.includes("fetch('/api/health'"), 'Service Worker deve invalidar o cache antigo e acordar o Render em segundo plano.');
assert(indexSource.includes('v=5.7.3'), 'HTML deve apontar para os assets novos e não reutilizar cache antigo.');
