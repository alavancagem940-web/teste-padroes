...
function chooseBundleFallback(matches, profile, usedSelections = new Set(), usedArchetypes = new Map()) {
  ...
  const rank = s => {
    ...
    return -dist*220 + (s.legs.length?s.score/s.legs.length:0)*.15 + Math.min(s.playerLegs,Number(profile.minPlayerLegs||0))*12;
  };
...
function ticketProfiles() {
  ...
  return {
    ...
    valor: {
      ...
      minP:.57, minFair:1.04, maxFair:2.25, minReliability:.70, minLegs:2, maxLegs:16, maxSameMatch:8, maxSameArchetype:8,
    ...
    arriscado: {
      ...
      minP:.53, maxP:null, minFair:1.04, maxFair:2.45, minReliability:.76, minLegs:3, maxLegs:20, maxSameMatch:8, maxSameArchetype:10,
    ...
    'muito-arriscado': {
      ...
      minP:.52, maxP:null, minFair:1.04, maxFair:2.50, minReliability:.78, minLegs:3, maxLegs:22, maxSameMatch:8, maxSameArchetype:12,
    ...
    jackpot: {
      ...
      minP:.45, maxP:null, minFair:1.04, maxFair:2.80, minReliability:.80, minLegs:4, maxLegs:24, maxSameMatch:8, maxSameArchetype:16,
...
const INTERNAL_ODD_BANDS = {
  ...
  valor: [
    { min:2.00, max:2.66, center:2.32, maxLegs:12 },
    { min:2.67, max:3.33, center:3.00, maxLegs:14 },
    { min:3.34, max:4.00, center:3.67, maxLegs:16 }
  ],
  arriscado: [
    { min:4.00, max:4.66, center:4.33, maxLegs:16 },
    { min:4.67, max:5.33, center:5.00, maxLegs:18 },
    { min:5.34, max:6.00, center:5.67, maxLegs:20 }
  ],
  'muito-arriscado': [
    { min:6.00, max:6.66, center:6.33, maxLegs:18 },
    { min:6.67, max:7.33, center:7.00, maxLegs:20 },
    { min:7.34, max:8.00, center:7.67, maxLegs:22 }
  ],
  jackpot: [
    { min:8.00, max:9.99, center:8.90, maxLegs:20 },
    { min:10.00, max:11.99, center:10.90, maxLegs:22 },
    { min:12.00, max:Number.POSITIVE_INFINITY, center:14.00, maxLegs:24 }
  ]
};
  ...
  for (const definition of definitions) {
    ...
    for (let bandIndex = 0; bandIndex < wanted; bandIndex++) {
      ...
      if (!ticket) {
        ...
        const retry = {
          ...baseRetry,
          maxLegs:Math.max(Number(baseRetry.maxLegs || 1), definition.key === 'jackpot' ? 24 : definition.key === 'muito-arriscado' ? 22 : definition.key === 'arriscado' ? 20 : definition.key === 'valor' ? 16 : 5),
          maxSameMatch:Math.max(Number(baseRetry.maxSameMatch || 1), definition.key === 'conservador' ? 3 : 8),
          maxSameArchetype:Math.max(Number(profile.maxSameArchetype || 2), 12),
          maxSameFamily:Math.max(Number(profile.maxSameFamily || 2), 12),
