...
const SPORTSDB_LEAGUE_NAMES = {
  ...
  mls: ['Major League Soccer','MLS','American Major League Soccer'],
  'champions-league': ['UEFA Champions League','Champions League'],
  'europa-league': ['UEFA Europa League','Europa League'],
  'conference-league': ['UEFA Europa Conference League','Europa Conference League','Conference League'],
  libertadores: ['CONMEBOL Libertadores','Copa Libertadores','Libertadores'],
  'sul-americana': ['CONMEBOL Sudamericana','Copa Sudamericana','Sul-Americana','Sudamericana'],
  'copa-do-brasil': ['Copa do Brasil','Copa do Brazil']
};
...
const PRIMARY_ESPN_SCOPE_BY_COUNTRY = {
  ...
  Argentina:'arg.1', Greece:'gre.1', Austria:'aut.1', 'Saudi Arabia':'ksa.1', 'United States':'usa.1', USA:'usa.1'
};
...
function scopeFromSeasonSlug(value='') {
  ...
  const rules = [
    ...
    [/saudi pro league/, 'ksa.1'],
    [/uefa champions league|champions league/, 'uefa.champions'], [/uefa europa league|europa league/, 'uefa.europa'],
    [/uefa europa conference league|europa conference league|conference league/, 'uefa.europa.conf'],
    [/copa libertadores|conmebol libertadores|libertadores/, 'conmebol.libertadores'],
    [/copa sudamericana|conmebol sudamericana|sul americana|sudamericana/, 'conmebol.sudamericana']
  ];
...
function countryFromScope(scope='') {
  ...
  return ({bra:'Brazil',eng:'England',esp:'Spain',ger:'Germany',ita:'Italy',fra:'France',sco:'Scotland',por:'Portugal',ned:'Netherlands',bel:'Belgium',tur:'Turkey',arg:'Argentina',gre:'Greece',aut:'Austria',ksa:'Saudi Arabia',uefa:'Europe',conmebol:'South America'})[prefix] || '';
}

function looksLikeWomensCompetition(value='') {
  const text = normalizeText(value);
  return /\b(?:women|womens|woman|female|feminino|feminina|femenino|femenina|femeni|femminile|frauen|liga f|wsl|division 1 feminine|d1 feminine|nwsl)\b/.test(text);
}
...
function normalizeEspnAllFavoriteEvent(event) {
  ...
  row.isWomensFootball = looksLikeWomensCompetition(`${seasonSlug} ${explicitLeague} ${event?.gender || ''}`);
  return row;
}
...
function isWomensSportsDbEvent(raw) {
  const gender = normalizeText(raw?.strGender || raw?.strLeagueGender || raw?.strEventGender || '');
  if (/^(female|women|womens|feminino|feminina)$/.test(gender)) return true;

  const competition = normalizeText(`${raw?.strLeague || ''} ${raw?.strLeagueAlternate || ''}`);
  // O endpoint diário do TheSportsDB mistura competições masculinas e femininas.
  // Como o antigo match aceitava "contém La Liga", nomes como "Spanish La Liga Women"
  // podiam ser classificados como La Liga masculina.
  return /\b(?:women|womens|woman|female|feminino|feminina|femenino|femenina|femeni|femminile|frauen|liga f|wsl|division 1 feminine|d1 feminine)\b/.test(competition);
}

export function sportsDbLeagueMatches(raw, leagueKey) {
  if (isWomensSportsDbEvent(raw)) return false;
...
export function normalizeSportsDbEvent(e, leagueKey) {
  ...
  return {
    ...
    isWomensFootball:isWomensSportsDbEvent(e),
