...
function candidateSignals(c) {
  ...
  if (family === 'Cartões' || family === 'Faltas') {
    out.add('duel-intensity');
    // Faltas/cartões de um lado costumam conversar com a pressão ofensiva do adversário:
    // o time que passa mais tempo defendendo tende a fazer mais contatos e receber mais cartões.
    if (side) out.add(`${side === 'home' ? 'away' : 'home'}-attack`);
  }
  return out;
}
...
function combinedPriceForLegs(legs, profile) {
  // A odd exibida do bilhete é a multiplicação das odds de cada seleção.
  // Correlação entre seleções da mesma partida NÃO reduz a cotação aqui; ela já
  // é tratada separadamente na probabilidade conjunta/odd justa. Isso mantém a
  // conta auditável pelo usuário (ex.: 1.03 × 1.03 × 1.07 = 1.14).
  return legs.reduce((p,l) => p * candidatePrice(l), 1);
}
...
function respectsFavoriteSide(c, match) {
  if (!String(match?.leagueKey || '').startsWith('favorite-')) return true;
  const favoriteSide = match?.favoriteSide;
  if (favoriteSide !== 'home' && favoriteSide !== 'away') return true;
  const side = attackingSide(c);
  if (!side) return true;
  return side === favoriteSide;
}

function allCandidates(match, profile, forceFallback = false) {
  const real = (match.opportunities || []).map(c => normalizeCandidate(c, match));
  const estimated = (match.estimatedMarkets || []).filter(c => !c.catalogOnly).map(c => normalizeCandidate(c, match));
  const source = [...real, ...estimated].filter(c => respectsFavoriteSide(c, match));
  let pool = source
    ...
    .filter(c => profile.minReliability == null || Number(c.reliability || 0) >= (c.fallbackEstimated ? Math.min(Number(profile.minReliability), .56) : c.modeledRate ? Math.min(Number(profile.minReliability), .70) : profile.minReliability))
    .filter(c => !c.isPlayerMarket || profile.minPlayerP == null || Number(c.safeProbability) >= profile.minPlayerP)
    .filter(c => !c.isPlayerMarket || profile.minPlayerReliability == null || Number(c.reliability || 0) >= (c.modeledRate ? Math.min(Number(profile.minPlayerReliability), .70) : profile.minPlayerReliability))
...
function chooseStrictRangeLegs(matches, profile, usedSelections = new Set(), usedArchetypes = new Map()) {
  ...
  for (const item of items) {
    ...
    states = next.sort((a,b) => {
      ...
      const legPenalty = Number(profile.legCountPenalty || 8);
      const qa = (a.legs.length ? a.score / a.legs.length : 0) + Math.min(a.legs.length, profile.minLegs) * 18 + Math.log(Math.max(a.safeJoint, .000001)) * hitWeight - Math.abs(Math.min(a.combined, profile.targetMax) - center) * 2 + Math.min(a.playerLegs, Number(profile.minPlayerLegs || 0)) * 20 - Math.max(0, a.legs.length - profile.minLegs) * legPenalty;
      const qb = (b.legs.length ? b.score / b.legs.length : 0) + Math.min(b.legs.length, profile.minLegs) * 18 + Math.log(Math.max(b.safeJoint, .000001)) * hitWeight - Math.abs(Math.min(b.combined, profile.targetMax) - center) * 2 + Math.min(b.playerLegs, Number(profile.minPlayerLegs || 0)) * 20 - Math.max(0, b.legs.length - profile.minLegs) * legPenalty;
      return qb - qa;
    }).slice(0, beamWidth);
  }
  ...
  return finalists.sort((a,b) => {
    const hitWeight = Number(profile.hitRateWeight || 0);
    const legPenalty = Number(profile.legCountPenalty || 8);
    const qa = (a.legs.length ? a.score / a.legs.length : 0) + Math.log(Math.max(a.safeJoint, .000001)) * hitWeight - Math.abs(a.combined - center) * Number(profile.targetClosenessPenalty || 4) + Math.min(a.playerLegs, Number(profile.minPlayerLegs || 0)) * 25 - Math.max(0, a.legs.length - profile.minLegs) * legPenalty;
    const qb = (b.legs.length ? b.score / b.legs.length : 0) + Math.log(Math.max(b.safeJoint, .000001)) * hitWeight - Math.abs(b.combined - center) * Number(profile.targetClosenessPenalty || 4) + Math.min(b.playerLegs, Number(profile.minPlayerLegs || 0)) * 25 - Math.max(0, b.legs.length - profile.minLegs) * legPenalty;
    return qb - qa;
  })[0]?.legs || [];
}
...
function chooseTargetRangeRecovery(matches, profile, usedSelections = new Set(), usedArchetypes = new Map()) {
  ...
  const stateRank = state => {
    ...
    return -distance * 180 + avgScore * .22 + playerBoost - Math.max(0, state.legs.length - Number(profile.minLegs || 1)) * Number(profile.legCountPenalty || 8);
  };
...
function chooseBundleFallback(matches, profile, usedSelections = new Set(), usedArchetypes = new Map()) {
  const targetMin = Number(profile.targetMin || 1);
  const targetMax = Number(profile.targetMax || Number.POSITIVE_INFINITY);
  const center = Number(profile.targetCenter || (Number.isFinite(targetMax) ? (targetMin + targetMax) / 2 : targetMin * 1.18));
  const maxLegs = Number(profile.maxLegs || 1);
  const maxSameMatch = Number(profile.maxSameMatch || 1);

  const bundleGroups = [];
  for (const match of matches) {
    const base = allCandidates(match, profile, true).map(c => {
      const repeatPenalty = usedSelections.has(`${c.matchKey}|${c.selection}`) ? Number(profile.repeatPenalty || 0) * .08 : 0;
      const archetypePenalty = Number(usedArchetypes.get(c.archetype) || 0) * Number(profile.archetypeRepeatPenalty || 0) * .06;
      return { c, score: profile.score(c) - repeatPenalty - archetypePenalty };
    });
    if (!base.length) continue;
    const chosen = [];
    const add = row => { if (row && !chosen.some(x => x.c === row.c)) chosen.push(row); };
    [...base].sort((a,b)=>b.score-a.score).slice(0,6).forEach(add);
    [...base].sort((a,b)=>candidatePrice(b.c)-candidatePrice(a.c) || b.score-a.score).slice(0,6).forEach(add);
    const fam = new Map();
    for (const row of [...base].sort((a,b)=>b.score-a.score)) if (!fam.has(row.c.family)) fam.set(row.c.family,row);
    [...fam.values()].slice(0,5).forEach(add);
    const items = chosen.slice(0,14);

    const bundles = [{ legs:[], odd:1, score:0, playerLegs:0 }];
    const dfs = (start, legs, odd, score, playerLegs, families, archetypes) => {
      if (legs.length >= maxSameMatch) return;
      for (let i=start;i<items.length;i++) {
        const {c,score:rowScore}=items[i];
        if (legs.some(l=>!pairCompatible(l,c))) continue;
        if (legs.length && !legs.some(l=>pairCoherence(l,c)>0)) continue;
        if (Number(families.get(c.family)||0) >= Number(profile.maxSameFamily ?? 99)) continue;
        if (Number(archetypes.get(c.archetype)||0) >= Number(profile.maxSameArchetype ?? 99)) continue;
        const nextOdd=odd*candidatePrice(c);
        if (nextOdd > targetMax + 1e-9) continue;
        const nf=new Map(families), na=new Map(archetypes);
        nf.set(c.family,Number(nf.get(c.family)||0)+1);
        na.set(c.archetype,Number(na.get(c.archetype)||0)+1);
        const coherence=legs.reduce((sum,l)=>sum+Math.max(0,pairCoherence(l,c)),0)*Number(profile.coherenceWeight||2.5);
        const next={ legs:[...legs,c], odd:nextOdd, score:score+rowScore+coherence, playerLegs:playerLegs+(c.isPlayerMarket?1:0) };
        bundles.push(next);
        dfs(i+1,next.legs,nextOdd,next.score,next.playerLegs,nf,na);
      }
    };
    dfs(0,[],1,0,0,new Map(),new Map());

    const bucketCount=72;
    const cap=Number.isFinite(targetMax)?Math.max(targetMax,1.05):Math.max(center*1.6,16);
    const denom=Math.max(Math.log(cap),.0001);
    const bucketed=Array.from({length:bucketCount},()=>[]);
    for (const b of bundles) {
      const idx=Math.max(0,Math.min(bucketCount-1,Math.floor((Math.log(Math.max(1,b.odd))/denom)*bucketCount)));
      bucketed[idx].push(b);
    }
    const kept=bucketed.flatMap(bucket=>bucket.sort((a,b)=>b.score-a.score || a.legs.length-b.legs.length).slice(0,4));
    bundleGroups.push(kept);
  }
  if (!bundleGroups.length) return [];

  let states=[{legs:[],combined:1,score:0,playerLegs:0}];
  const beam=1000;
  const rank = s => {
    const dist=s.combined<targetMin ? Math.log(targetMin/Math.max(s.combined,1)) : Math.abs(Math.log(Math.max(s.combined/Math.max(center,1.01),.000001)));
    return -dist*220 + (s.legs.length?s.score/s.legs.length:0)*.15 + Math.min(s.playerLegs,Number(profile.minPlayerLegs||0))*12 - Math.max(0, s.legs.length-Number(profile.minLegs||1))*Number(profile.legCountPenalty||8);
  };
  for (const bundles of bundleGroups) {
    const next=[];
    for (const state of states) {
      for (const bundle of bundles) {
        const legs=[...state.legs,...bundle.legs];
        if (legs.length>maxLegs) continue;
        const combined=state.combined*bundle.odd;
        if (combined>targetMax+1e-9) continue;
        next.push({legs,combined,score:state.score+bundle.score,playerLegs:state.playerLegs+bundle.playerLegs});
      }
    }
    ...
    for (const st of next) {
      const idx=Math.max(0,Math.min(bucketCount-1,Math.floor((Math.log(Math.max(1,st.combined))/denom)*bucketCount)));
      buckets[idx].push(st);
    }
    states=buckets.flatMap(bucket=>bucket.sort((a,b)=>rank(b)-rank(a)).slice(0,10)).sort((a,b)=>rank(b)-rank(a)).slice(0,beam);
  }
  let valid=states.filter(s=>s.legs.length>=Number(profile.minLegs||1)&&s.combined>=targetMin-1e-9&&s.combined<=targetMax+1e-9&&s.playerLegs>=Number(profile.minPlayerLegs||0));
  if (!valid.length) valid=states.filter(s=>s.legs.length>=Number(profile.minLegs||1)&&s.combined>=targetMin-1e-9&&s.combined<=targetMax+1e-9);
  return valid.sort((a,b)=>rank(b)-rank(a))[0]?.legs || [];
}

function chooseAccumulatorFallback(matches, profile, usedSelections = new Set(), usedArchetypes = new Map()) {
  const rows = [];
  const seen = new Set();
  for (const match of matches) {
    for (const c of allCandidates(match, profile, true)) {
      const id = `${c.matchKey}|${c.key}|${c.selection}`;
      if (seen.has(id)) continue;
      seen.add(id);
      const repeatPenalty = usedSelections.has(`${c.matchKey}|${c.selection}`) ? Number(profile.repeatPenalty || 0) * .10 : 0;
      const archetypePenalty = Number(usedArchetypes.get(c.archetype) || 0) * Number(profile.archetypeRepeatPenalty || 0) * .08;
      rows.push({ c, score: profile.score(c) - repeatPenalty - archetypePenalty });
    }
  }
  ...
  const strategies = [
    ...
    [...rows].sort((a,b) => candidatePrice(a.c) - candidatePrice(b.c) || b.score - a.score),
    [...rows].sort((a,b) => candidatePrice(b.c) - candidatePrice(a.c) || b.score - a.score),
    [...rows].sort((a,b) => ((candidatePrice(b.c)-1) * Math.max(.05,b.c.safeProbability||b.c.probability)) - ((candidatePrice(a.c)-1) * Math.max(.05,a.c.safeProbability||a.c.probability)) || b.score-a.score)
  ];

  const attempt = (ordered, rotate = 0) => {
    const list = rotate ? [...ordered.slice(rotate), ...ordered.slice(0, rotate)] : ordered;
    const legs = [];
    const families = new Map();
    const archetypes = new Map();
    let combined = 1;
    while (legs.length < maxLegs) {
      if (legs.length >= Number(profile.minLegs || 1) && combined >= targetMin - 1e-9 && combined <= targetMax + 1e-9) return legs;
      let best = null;
      let bestRank = -Infinity;
      for (let i=0;i<list.length;i++) {
        const row=list[i], c=row.c;
        if (legs.includes(c)) continue;
        if (!canAddSameMatch(legs,c,profile)) continue;
        if (Number(families.get(c.family)||0) >= Number(profile.maxSameFamily ?? 99)) continue;
        if (Number(archetypes.get(c.archetype)||0) >= Number(profile.maxSameArchetype ?? 99)) continue;
        const nextOdd=combined*candidatePrice(c);
        if (nextOdd > targetMax + 1e-9) continue;
        const remainingDistance = nextOdd < targetMin ? Math.log(targetMin/Math.max(nextOdd,1)) : Math.abs(Math.log(Math.max(nextOdd/Math.max(center,1.01),.000001)));
        const needGrowth = combined < targetMin ? Math.log(Math.max(candidatePrice(c),1.0001)) : 0;
        const same = legs.filter(l=>l.matchKey===c.matchKey);
        const coherence = same.reduce((sum,l)=>sum+Math.max(0,pairCoherence(l,c)),0);
        const rank = -remainingDistance*120 + needGrowth*28 + row.score*.04 + coherence*2 - i*.0001;
        if (rank > bestRank) { bestRank=rank; best=c; }
      }
      if (!best) break;
      legs.push(best);
      combined*=candidatePrice(best);
      families.set(best.family,Number(families.get(best.family)||0)+1);
      archetypes.set(best.archetype,Number(archetypes.get(best.archetype)||0)+1);
    }
    return legs.length >= Number(profile.minLegs||1) && combined >= targetMin-1e-9 && combined <= targetMax+1e-9 ? legs : [];
  };

  for (const ordered of strategies) {
    for (let rotate=0; rotate<Math.min(8, ordered.length); rotate++) {
      const legs=attempt(ordered,rotate);
...
function makeTicket(name, matches, profile, usedSelections, usedArchetypes) {
  ...
  if (profile.strictTargetRange && !legs.length) {
    legs = chooseBundleFallback(matches, profile, usedSelections, usedArchetypes);
  }
  if (profile.strictTargetRange && !legs.length) {
    legs = chooseAccumulatorFallback(matches, profile, usedSelections, usedArchetypes);
  }
  ...
  // A confiabilidade dos dados é exibida separadamente. Não faz sentido descontá-la
  // em TODAS as pernas e deixar um bilhete de odd ~5 com 1% de GREEN e odd justa 80.
  // Aplicamos uma única margem conservadora ao conjunto; assim chance/odd justa
  // continuam coerentes com as probabilidades que originaram as odds aproximadas.
  const avgReliabilityForTicket = legs.reduce((sum,l) => sum + Number(l.reliability || .75), 0) / legs.length;
  const reliabilityFactor = .90 + .10 * clamp(avgReliabilityForTicket, .55, 1);
  const jointProbability = clamp(modelJointProbability * reliabilityFactor, .000001, .999999);
  ...
  return {
    ...
    mixRecipe: profile.mixRecipe || null,
    legs
  };
}
...
function ticketProfiles() {
  ...
  return {
    conservador: {
      ...
      maxSameFamily:2, withinFamilyPenalty:5, withinArchetypePenalty:22, legCountPenalty:8,
    ...
    valor: {
      ...
      minP:.57, minFair:1.04, maxFair:2.25, minReliability:.70, minLegs:2, maxLegs:4, maxSameMatch:3, maxSameArchetype:3,
      ...
      maxSameFamily:3, legCountPenalty:10,
    ...
    arriscado: {
      ...
      minP:.53, maxP:null, minFair:1.04, maxFair:2.45, minReliability:.76, minLegs:3, maxLegs:6, maxSameMatch:3, maxSameArchetype:4,
      ...
      repeatPenalty:105, archetypeRepeatPenalty:38, maxSameFamily:4, legCountPenalty:11,
    ...
    'muito-arriscado': {
      ...
      minP:.52, maxP:null, minFair:1.04, maxFair:2.50, minReliability:.78, minLegs:4, maxLegs:7, maxSameMatch:4, maxSameArchetype:4,
      ...
      repeatPenalty:125, archetypeRepeatPenalty:44, maxSameFamily:4, legCountPenalty:12,
    ...
    jackpot: {
      ...
      minP:.45, maxP:null, minFair:1.04, maxFair:2.80, minReliability:.80, minLegs:4, maxLegs:8, maxSameMatch:4, maxSameArchetype:5,
      ...
      repeatPenalty:145, archetypeRepeatPenalty:50, maxSameFamily:5, legCountPenalty:13,
...
const INTERNAL_ODD_BANDS = {
  // Faixas internas: nunca são exibidas como "níveis". O limite de pernas impede
  // que o sistema force odds altas empilhando 15–24 seleções de 1.03/1.07.
  conservador: [
    { min:1.20, max:1.33, center:1.26, maxLegs:2 },
    { min:1.34, max:1.66, center:1.50, maxLegs:3 },
    { min:1.67, max:1.99, center:1.83, maxLegs:3 }
  ],
  valor: [
    { min:2.00, max:2.66, center:2.32, maxLegs:4 },
    { min:2.67, max:3.33, center:3.00, maxLegs:4 },
    { min:3.34, max:4.00, center:3.67, maxLegs:4 }
  ],
  arriscado: [
    { min:4.00, max:4.66, center:4.33, maxLegs:5 },
    { min:4.67, max:5.33, center:5.00, maxLegs:6 },
    { min:5.34, max:6.00, center:5.67, maxLegs:6 }
  ],
  'muito-arriscado': [
    { min:6.00, max:6.66, center:6.33, maxLegs:6 },
    { min:6.67, max:7.33, center:7.00, maxLegs:7 },
    { min:7.34, max:8.00, center:7.67, maxLegs:7 }
  ],
  jackpot: [
    { min:8.00, max:9.99, center:8.90, maxLegs:6 },
    { min:10.00, max:11.99, center:10.90, maxLegs:7 },
    { min:12.00, max:Number.POSITIVE_INFINITY, center:14.00, maxLegs:8 }
  ]
};
...
function profileBand(profile, key, bandIndex) {
  ...
  return {
    ...
    maxLegs:Number(band.maxLegs || profile.maxLegs || 1),
...
function applyTicketMixRecipe(profile, bandIndex) {
  // Os três bilhetes do mesmo perfil continuam com a mesma faixa de risco/odd,
  // mas recebem estilos de composição diferentes. Isso imita a variedade de
  // uma casa de apostas sem trocar a lógica de probabilidade do Bilhete Plus.
  const recipes = [
    {
      key:'equilibrado',
      preferredFamilies:new Set(['Resultado','Gols','Ambas marcam','Gols do time']),
      familyBonus:20,
      coherenceWeight:Math.max(Number(profile.coherenceWeight || 0), 2.8),
      maxSameFamily:Math.min(Number(profile.maxSameFamily ?? 99), 2)
    },
    {
      key:'mesmo-jogo',
      preferredFamilies:new Set(['Chutes no gol','Chutes','Resultado','Gols','Ambas marcam','Gols do time']),
      familyBonus:24,
      playerBonus:28,
      coherenceWeight:Math.max(Number(profile.coherenceWeight || 0), 4.4),
      maxSameMatch:Math.max(Number(profile.maxSameMatch || 1), 3)
    },
    {
      key:'mercados-variados',
      preferredFamilies:new Set(['Escanteios','Cartões','Faltas','Defesas','Chutes no gol','Chutes']),
      familyBonus:26,
      playerBonus:10,
      coherenceWeight:Math.max(Number(profile.coherenceWeight || 0), 3.2),
      maxSameFamily:Math.min(Number(profile.maxSameFamily ?? 99), 1),
      withinFamilyPenalty:Math.max(Number(profile.withinFamilyPenalty || 0), 18),
      repeatPenalty:Math.max(Number(profile.repeatPenalty || 0), 90)
    }
  ];
  const recipe = recipes[Math.max(0, Math.min(recipes.length - 1, Number(bandIndex || 0)))] || recipes[0];
  const baseScore = profile.score;
  return {
    ...profile,
    mixRecipe:recipe.key,
    coherenceWeight:recipe.coherenceWeight,
    maxSameMatch:recipe.maxSameMatch ?? profile.maxSameMatch,
    maxSameFamily:recipe.maxSameFamily ?? profile.maxSameFamily,
    withinFamilyPenalty:recipe.withinFamilyPenalty ?? profile.withinFamilyPenalty,
    repeatPenalty:recipe.repeatPenalty ?? profile.repeatPenalty,
    score:c => baseScore(c)
      + (recipe.preferredFamilies.has(c.family) ? recipe.familyBonus : 0)
      + (c.isPlayerMarket ? Number(recipe.playerBonus || 0) : 0)
  };
}
...
export function buildTicketGroups(matches, maxPerProfile = 3) {
  ...
  for (const definition of definitions) {
    ...
    for (let bandIndex = 0; bandIndex < wanted; bandIndex++) {
      let bandProfile = applyTicketMixRecipe(profileBand(profile, definition.key, bandIndex), bandIndex);
      ...
      if (!ticket) {
        const baseRetry = applyTicketMixRecipe(profileBand(profile, definition.key, bandIndex), bandIndex);
        const retry = {
          ...baseRetry,
          maxLegs:Number(baseRetry.maxLegs || profile.maxLegs || 1),
          maxSameMatch:Number(baseRetry.maxSameMatch || profile.maxSameMatch || 1),
          maxSameArchetype:Number(baseRetry.maxSameArchetype || profile.maxSameArchetype || 2),
          maxSameFamily:Number(baseRetry.maxSameFamily || profile.maxSameFamily || 2),
          beamWidth:Math.max(Number(baseRetry.beamWidth || 240), 700)
        };
        ticket = makeTicket(definition.name, prematchMatches, retry, new Set(), new Map());
      }
