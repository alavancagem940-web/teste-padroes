...
import { getLeagueOdds, getBrasileiraoOdds, getLeagueEvents, getLeagueScores, normalizeOddsEvent, normalizeOddsScore, findOddsGame, summarizeOdds, LEAGUES, MAJOR_FIVE, MAJOR_SECOND, MAJOR_CUPS } from './src/providers/oddsApi.js';
...
// Mercado dos Favoritos continua significando favorito NO JOGO; o filtro abaixo só remove competições não profissionais.
...
const AUTOMATIC_TIME_BANDS = new Set(['day-all','morning','afternoon','night']);
function hourInSaoPaulo(iso) {
...
function matchInAutomaticTimeBand(match, band = 'day-all') {
  if (band === 'day-all') return true;
  const hour = hourInSaoPaulo(match?.kickoff || match?.date || match?.commence_time);
  if (hour === null) return false;
  if (band === 'morning') return hour >= 6 && hour < 12;
  if (band === 'afternoon') return hour >= 12 && hour < 18;
  if (band === 'night') return hour >= 18 || hour < 6;
  return true;
}
function applyAutomaticTimeBand(data, band = 'day-all', skipTickets = false) {
  const safeBand = AUTOMATIC_TIME_BANDS.has(band) ? band : 'day-all';
  if (safeBand === 'day-all') return { ...data, timeBand:safeBand };
  const matches = (data.matches || []).filter(match => matchInAutomaticTimeBand(match, safeBand));
  return { ...data, timeBand:safeBand, count:matches.length, matches, ...maybeBuildTicketPayload(matches, skipTickets) };
}
...
const CLUB_STRENGTH = new Map(Object.entries({
  'real madrid':0.98,'barcelona':0.95,'atletico madrid':0.88,'athletic bilbao':0.78,'sevilla':0.72,'villarreal':0.76,
  'manchester city':0.96,'liverpool':0.94,'arsenal':0.93,'chelsea':0.84,'manchester united':0.80,'tottenham hotspur':0.80,'newcastle united':0.80,'aston villa':0.78,
  'bayern munich':0.96,'bayern munchen':0.96,'borussia dortmund':0.86,'bayer leverkusen':0.90,'bayer 04 leverkusen':0.90,'rb leipzig':0.82,
  'inter':0.93,'inter milan':0.93,'internazionale':0.93,'ac milan':0.88,'juventus':0.88,'napoli':0.89,'roma':0.80,'lazio':0.76,'atalanta':0.84,
  'paris saint germain':0.96,'psg':0.96,'marseille':0.80,'monaco':0.82,'lyon':0.76,'lille':0.79,
  'flamengo':0.92,'palmeiras':0.92,'botafogo':0.84,'atletico mineiro':0.84,'fluminense':0.80,'corinthians':0.80,'sao paulo':0.82,'cruzeiro':0.80,
  'inter miami':0.84,'los angeles fc':0.82,'lafc':0.82,'columbus crew':0.80,'seattle sounders':0.80
}));
function clubStrength(name='') { return CLUB_STRENGTH.get(normalizeName(name)) ?? (isFavoriteClub(name) ? .78 : .50); }
function h2hEdgeFromFixture(fixture) {
  const h = Number(fixture?.h2h?.homeWinRate ?? fixture?.headToHead?.homeWinRate);
  const a = Number(fixture?.h2h?.awayWinRate ?? fixture?.headToHead?.awayWinRate);
  return Number.isFinite(h) && Number.isFinite(a) ? clamp(h-a,-1,1) : 0;
}
...
function favoriteAnalysisFromForms(fixture, homeForm={}, awayForm={}) {
  ...
  // Favoritismo não é sinônimo de mando. O mando é apenas um fator pequeno.
  // A maior parte do peso vem de força estrutural do clube + momento/forma + saldo.
  // H2H só entra quando a fonte realmente fornecer esse dado; na ausência dele é neutro.
  const identityEdge = identityFavoriteEdge(fixture, homeForm, awayForm) * .20;
  const strengthEdge = (clubStrength(fixture.home?.name) - clubStrength(fixture.away?.name)) * .78;
  const formEdge = (hp-ap)/3*.46;
  const goalDiffEdge = (hgd-agd)/3*.20;
  const homeAdvantage = .045;
  const h2hEdge = h2hEdgeFromFixture(fixture) * .12;
  const edge = clamp(formEdge + goalDiffEdge + strengthEdge + homeAdvantage + h2hEdge + identityEdge, -.72, .72);
  ...
  return {
    ...
    favoriteSide: home >= away ? 'home' : 'away',
    favoriteTeam: home >= away ? (fixture.home?.name || '') : (fixture.away?.name || ''),
    favoriteFactors:{clubStrength:{home:clubStrength(fixture.home?.name),away:clubStrength(fixture.away?.name)},formEdge,goalDiffEdge,homeAdvantage,h2hEdge},
...
async function favoriteMarketDashboard(date, includePlayers = false, skipTickets = false, includeWomen = false) {
  ...
  let candidates = sortFavoriteCandidates(onlyPrematch((publicResult.fixtures || []).filter(f => favoriteFixtureEligible(f) && (includeWomen || !isWomensFixture(f))))).slice(0, 24);
  ...
  if (!candidates.length && process.env.ODDS_API_KEY) {
    try {
      ...
      candidates = sortFavoriteCandidates(onlyPrematch((oddsFallback.fixtures || []).filter(f => favoriteFixtureEligible(f) && (includeWomen || !isWomensFixture(f))))).slice(0, 24);
  ...
  return {
    ...
    sourceStatus:{
      ...
      excluded:'Champions League, Libertadores e Sul-Americana; também universitário, base/reservas e amador',
      fallbackActive:true,
      warning:discoveryWarnings.filter(Boolean).slice(0,3).join(' | ') || null
    },
    count:matches.length,
    includeWomen:Boolean(includeWomen),
    matches,
...
async function womensFootballDashboard(date, includePlayers = false, skipTickets = false) {
  const publicResult = await getPublicFavoriteFixtures(date).catch(error => ({ fixtures:[], source:'nenhuma', errors:[error.message] }));
  const candidates = onlyPrematch((publicResult.fixtures || []).filter(isWomensFixture));
  let matches = candidates.map(attachFavoritePublicAdvanced);
  let playerStatus = 'sob demanda';
  if (includePlayers && matches.length) {
    const enriched = await enrichWithPlayerMarkets(matches, date);
    matches = enriched.matches;
  ...
  return {
    date,
    leagueKey:'womens-football',
    sourceStatus:{
      mode:'Futebol feminino • partidas públicas + odds estimadas',
      footballData:'não usado', oddsApi:'não obrigatório', publicFixtures:publicResult.source || 'fonte pública',
      apiFootball:process.env.API_FOOTBALL_KEY ? 'reservada para jogadores/ao vivo' : 'não configurada',
      jogadores:playerStatus, fallbackActive:true,
      warning:(publicResult.errors || []).filter(Boolean).slice(0,3).join(' | ') || null
    },
    count:matches.length,
    matches,
    ...maybeBuildTicketPayload(matches, skipTickets),
    disclaimer:'Esta opção mostra somente partidas identificadas como futebol feminino. Sem odd real, o Bilhete Plus usa odd justa/estimada pelo modelo próprio.'
  };
}


function womensFixtureMatchesFilter(fixture, leagueKey) {
  if (!isWomensFixture(fixture)) return false;
  const text = normalizeName(`${fixture?.league?.name || ''} ${fixture?.league?.country || ''} ${fixture?.seasonSlug || ''} ${fixture?.espnScope || ''}`);
  const byLeague = {
    'premier-league': /england|english|wsl|women super league/,
    'la-liga': /spain|spanish|liga f|femen/,
    'serie-a': /italy|italian|serie a fem|femminile/,
    bundesliga: /germany|german|frauen|bundesliga f/,
    'ligue-1': /france|french|division 1 feminine|d1 feminine/,
    brasileirao: /brazil|brasil|brasileir.*femin/,
    mls: /united states|usa|nwsl|national women soccer league/,
    'saudi-pro-league': /saudi/
  };
  if (leagueKey === 'market-favorites') return true;
  if (leagueKey === 'all-major') return ['premier-league','la-liga','serie-a','bundesliga','ligue-1'].some(key => byLeague[key].test(text));
  return Boolean(byLeague[leagueKey]?.test(text));
}

async function addWomensMatchesToDashboard(data, date, leagueKey, includePlayers = false, skipTickets = false) {
  if (!data || leagueKey === 'womens-football') return { ...data, includeWomen:true };
  const publicResult = await getPublicFavoriteFixtures(date).catch(() => ({ fixtures:[] }));
  const extraFixtures = onlyPrematch((publicResult.fixtures || []).filter(f => womensFixtureMatchesFilter(f, leagueKey)));
  if (!extraFixtures.length) return { ...data, includeWomen:true };
  const existing = new Set((data.matches || []).map(m => `${normalizeName(m.home?.name)}|${normalizeName(m.away?.name)}|${m.kickoff || m.date || ''}`));
  let extras = extraFixtures.map(attachFavoritePublicAdvanced).filter(m => !existing.has(`${normalizeName(m.home?.name)}|${normalizeName(m.away?.name)}|${m.kickoff || m.date || ''}`));
  let matches = [...(data.matches || []), ...extras];
  let playerStatus = data.sourceStatus?.jogadores || 'sob demanda';
  if (includePlayers && extras.length) {
    const enriched = await enrichWithPlayerMarkets(matches, date);
    matches = enriched.matches;
    playerStatus = enriched.status;
  }
  return {
    ...
    count:matches.length,
    matches,
    sourceStatus:{ ...(data.sourceStatus || {}), jogadores:playerStatus },
    ...maybeBuildTicketPayload(matches, skipTickets)
  };
}
...
async function publicDashboardOnly(date, leagueKey, includePlayers = false, skipTickets = false) {
  ...
  const group = leagueKey === 'all-major' ? MAJOR_FIVE : leagueKey === 'all-second' ? MAJOR_SECOND : leagueKey === 'major-cups' ? MAJOR_CUPS : null;
...
async function dashboard(date, leagueKey, includePlayers = false, forcePublic = false, skipTickets = false, includeWomen = false) {
  if (leagueKey === 'womens-football') return womensFootballDashboard(date, includePlayers, skipTickets);
  if (leagueKey === 'market-favorites') return favoriteMarketDashboard(date, includePlayers, skipTickets, includeWomen);
  if (forcePublic) {
    const base = await publicDashboardOnly(date, leagueKey, includePlayers, skipTickets);
    return includeWomen ? addWomensMatchesToDashboard(base, date, leagueKey, includePlayers, skipTickets) : { ...base, includeWomen:false };
  }
  ...
  const api = { fixtures:[], error:null };
  if (leagueKey === 'all-major' || leagueKey === 'all-second' || leagueKey === 'major-cups') {
    const group = leagueKey === 'all-major' ? MAJOR_FIVE : leagueKey === 'all-second' ? MAJOR_SECOND : MAJOR_CUPS;
...
const server = http.createServer(async (req, res) => {
  try {
    ...
    if (url.pathname === '/api/dashboard') {
      ...
      const requestedBand = url.searchParams.get('timeBand') || 'day-all';
      const timeBand = AUTOMATIC_TIME_BANDS.has(requestedBand) ? requestedBand : 'day-all';
      if (!/^\d{4}-\d{2}-\d{2}$/.test(date)) return json(res, 400, { error: 'Data inválida. Use YYYY-MM-DD.' });
      // Com faixa específica, primeiro carrega as partidas sem montar bilhetes; depois filtra e só então roda os cinco perfis.
      const includeWomen = ['1','true','yes'].includes(String(url.searchParams.get('includeWomen') || '').toLowerCase());
      let base = await dashboard(date, league, includePlayers, forcePublic, fast || timeBand !== 'day-all', includeWomen);
      if (includeWomen && league !== 'womens-football' && league !== 'market-favorites' && !base.includeWomen) base = await addWomensMatchesToDashboard(base, date, league, includePlayers, fast || timeBand !== 'day-all');
      if (!includeWomen && league !== 'womens-football') base = { ...base, includeWomen:false, matches:(base.matches || []).filter(m => !isWomensFixture(m)) };
      if (!includeWomen && league !== 'womens-football') base = { ...base, count:base.matches.length, ...maybeBuildTicketPayload(base.matches, fast || timeBand !== 'day-all') };
      return json(res, 200, applyAutomaticTimeBand(base, timeBand, fast));
    }
