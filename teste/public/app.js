...
let playerMarketLoadStatus = {};
let playerMarketMetaByMatch = {};
let playerMarketLoading = new Map();
let activePlayerFocusByMatch = {};
let activeMarketFilter = 'todos';
let activeBuilderMarketCategory = 'popular';
let liveFixtures = [];
let liveSnapshots = [];
let liveTimer = null;
...
function clearGameState() {
  ...
  playerMarketLoadStatus = {};
  playerMarketMetaByMatch = {};
  playerMarketLoading = new Map();
  activePlayerFocusByMatch = {};
...
async function wakeBackend({ updateUI = false } = {}) {
  // O shell do PWA pode abrir do cache enquanto o Render Free ainda está dormindo.
  // Mantemos UMA chamada de aquecimento viva em segundo plano; abortá-la em 3s fazia
  // o usuário tocar em "Analisar jogos" antes do servidor terminar de acordar.
  if (backendLastReadyAt && Date.now() - backendLastReadyAt < 4 * 60 * 1000) return true;
  if (!backendWarmupPromise) {
    backendWarmupPromise = (async () => {
      ...
      try {
        ...
        if (updateUI) $('#health').textContent = 'Fontes em segundo plano';
        return false;
      } finally {
        clearTimeout(timer);
...
async function health() { return wakeBackend({ updateUI:true }); }
...
function legDisplayedOdd(l) {
  const value = l?.oddType === 'real' && l?.odd ? l.odd : (l?.displayOdd || l?.fairOdd || 1);
  return Math.max(1, Number(value) || 1);
}

function groupTicketLegsByMatch(legs = []) {
  const groups = [];
  const byMatch = new Map();
  for (const leg of legs) {
    const match = String(leg?.match || 'Partida').trim() || 'Partida';
    let group = byMatch.get(match);
    if (!group) {
      group = { match, legs: [] };
      byMatch.set(match, group);
      groups.push(group);
    }
    group.legs.push(leg);
  }
...
function renderTicketMatchGroups(legs = []) {
  const groups = groupTicketLegsByMatch(legs);
  if (!groups.length) return '';
  return `<div class="ticket-match-groups">${groups.map(group => {
    const groupOdd = group.legs.reduce((acc, leg) => acc * legDisplayedOdd(leg), 1);
    const allReal = group.legs.every(leg => leg.oddType === 'real' && leg.odd);
    const anyReal = group.legs.some(leg => leg.oddType === 'real' && leg.odd);
    const oddLabel = allReal ? 'Odd do jogo' : anyReal ? 'Odd combinada' : 'Odd aprox.';
    return `<section class="ticket-match-block">
      <div class="ticket-match-head">
        <div><strong class="ticket-match-name">${safe(group.match)}</strong><small>${group.legs.length} ${group.legs.length === 1 ? 'seleção' : 'seleções'}</small></div>
        <span class="ticket-match-odd"><small>${safe(oddLabel)}</small><strong>${money(groupOdd)}</strong></span>
      </div>
      <ul>${group.legs.map(l => `<li><strong>${safe(l.selection)}</strong><small>${legPriceText(l)}</small></li>`).join('')}</ul>
    </section>`;
  }).join('')}</div>`;
}
...
function ticketCard(t, profileKey, optionIndex) {
  ...
  return `<article class="ticket-slide profile-${safe(profileKey)}" data-profile="${safe(profileKey)}" data-option="${optionIndex}" data-ticket-number="${ticketNo}">
      <div class="ticket ticket-slide-card">
        ...
        ${renderTicketMatchGroups(t.legs)}
...
function matchKey(m) {
  return `${builderMarketText(m?.homeTeam)}|${builderMarketText(m?.awayTeam)}`;
}
function richerTicketMatchFor(m) {
  if (!m || !ticketDashboard?.matches?.length) return null;
  const key = matchKey(m);
  return ticketDashboard.matches.find(x => matchKey(x) === key) || null;
}
function allMarketsForMatch(index) {
  const m = dashboard?.matches?.[index]; if (!m) return [];
  const richer = richerTicketMatchFor(m);
  const extra = playerMarketsByMatch[index] || [];
  // O Montar meu bilhete não pode ficar preso ao dashboard mais pobre da aba Jogos.
  // Quando os Automáticos já carregaram props/mercados adicionais da mesma partida,
  // reaproveitamos esses mercados aqui também. E oportunidades reais nunca somem.
  const combined = [
    ...baseCandidates(m),
    ...(m.opportunities || []),
    ...(m.estimatedMarkets || []),
    ...(richer?.opportunities || []),
    ...(richer?.estimatedMarkets || []),
    ...extra
  ];
  const seen = new Set();
  return combined.filter(x => {
    const k = `${builderMarketText(x.market)}|${builderMarketText(x.key)}|${builderMarketText(x.selection)}`;
...
function marketCategory(m) {
  ...
  if (text.includes('gol') || text.includes('1x2') || text.includes('dupla chance') || text.includes('ambas') || text.includes('resultado') || text.includes('intervalo/final') || text.includes('handicap')) return 'gols';
  return 'outros';
}

function houseMarketTitle(name='') {
  const raw = String(name || 'Mercado');
  const map = {
    '1X2':'Resultado Final',
    'Gols':'Total de Gols',
    'Dupla chance':'Chance Dupla',
    'Ambas marcam':'Ambas as equipes marcam',
    'Gols do time':'Total de gols da equipe'
  };
  return map[raw] || raw;
}

function marketRows(markets, matchIndex, filter='todos', limit=220) {
  ...
  for (const market of rows) {
    const key = String(market.market || 'Outros');
    if (!grouped.has(key)) grouped.set(key, []);
    grouped.get(key).push(market);
  }
  return `<div class="house-market-groups">${[...grouped.entries()].map(([family,familyRows],groupIndex)=>`<details class="house-market-group" ${groupIndex < 5 ? 'open' : ''}><summary><strong>${safe(houseMarketTitle(family))}</strong><span>${familyRows.length} opção(ões)</span></summary><div class="market-table">${familyRows.map(m => {
    const idx = (markets || []).indexOf(m);
    const real = m.oddType === 'real' && m.odd;
    return `<div class="market-row"><div class="market-main"><span class="market-family">${safe(houseMarketTitle(m.market))}</span><strong>${safe(m.selection)}</strong><small>${safe(m.source || 'Modelo')} • confiança ${safe(m.confidence || (m.reliability >= .9 ? 'alta' : 'média'))}</small></div><div class="market-numbers"><b>${pct(m.probability)}</b><span>justa ${money(m.fairOdd || (1/Math.max(.001,m.probability)))}</span>${real ? `<em>real ${money(m.odd)}</em>` : ''}</div><button class="add-market" data-match="${matchIndex}" data-market-index="${idx}" aria-label="Adicionar ao bilhete">+</button></div>`;
...
function renderMatch(m, index) {
  ...
  const estimated = (m.estimatedMarkets || []).filter(x=>!x.catalogOnly && x.probability>=.65).sort((a,b)=>b.probability-a.probability)[0];
...
function renderMarketDetail(index) {
  ...
  const playerRows = markets.filter(row => marketCategory(row) === 'jogadores');
  const playerMode = activeMarketFilter === 'jogadores';
  const playerStatus = playerMarketLoadStatus[index] || '';
  const content = playerMode && !playerRows.length
    ? `<div class="empty player-category-state">${safe(playerStatus || (playerMarketLoading.has(index) ? 'Carregando mercados de jogadores…' : 'Buscando elenco e mercados de jogadores…'))}</div>`
    : marketRows(markets,index,activeMarketFilter);
  $('#marketDetail').innerHTML = `<section class="card market-detail"><div class="market-match-head"><div><span class="eyebrow">${safe(m.leagueLabel)}</span><h3>${safe(m.homeTeam)} x ${safe(m.awayTeam)}</h3><small>${markets.length} mercados disponíveis</small></div><button class="secondary load-player-market" data-index="${index}">${playersLoaded ? `${playersLoaded} mercados de jogadores` : (playerMarketLoading.has(index) ? 'Carregando jogadores…' : playerStatus ? 'Tentar novamente' : 'Carregar jogadores')}</button></div>${content}</section>`;
  wireMarketRows();
...
async function loadPlayerMarkets(index, btn) {
  const m = dashboard?.matches?.[index];
  const richer = richerTicketMatchFor(m);
  const f = m?.apiFixture || richer?.apiFixture || null;
  if (!m) return [];
  if (playerMarketLoading.has(index)) return playerMarketLoading.get(index);
  if (btn) { btn.disabled = true; btn.textContent = 'Carregando…'; }
  playerMarketLoadStatus[index] = 'Carregando mercados de jogadores…';
  const job = (async()=>{
    try {
      // Mesmo quando a aba Jogos veio de uma fonte pública e não trouxe IDs da
      // API-Football, enviamos os nomes. O servidor tenta localizar a mesma partida
      // por data + times e só então busca chutes/faltas/defesas dos jogadores.
      const params = {
        date:$('#date').value,
        leagueKey:m.leagueKey === 'market-favorites' || String(m.leagueKey || '').startsWith('favorite-') ? 'market-favorites' : ($('#league').value || m.leagueKey || 'market-favorites'),
      ...
      if (f) Object.assign(params, { homeTeamId:f.homeTeamId, awayTeamId:f.awayTeamId, leagueId:f.leagueId, leagueCountry:f.leagueCountry || '' });
      const publicFixture = m.publicFixture || m.favoriteFixture || null;
      if (publicFixture?.provider === 'espn-public' && publicFixture?.espnScope && publicFixture?.home?.id && publicFixture?.away?.id) {
        Object.assign(params, {
          publicFixtureId:publicFixture.fixtureId || '', publicScope:publicFixture.espnScope,
          publicHomeId:publicFixture.home.id, publicAwayId:publicFixture.away.id
        });
      }
      ...
      let r;
      try { r = await fetch(`/api/pregame/player-markets?${q}`, { signal:controller.signal }); }
      catch (error) {
        if (error?.name === 'AbortError') throw new Error('Jogadores demoraram demais para responder. O restante do app continua funcionando; tente novamente daqui a pouco.');
        throw error;
      } finally { clearTimeout(timer); }
      const data = await r.json();
      if(!r.ok) throw new Error(data.error || 'Falha ao carregar jogadores');
      playerMarketsByMatch[index] = data.markets || [];
      playerMarketMetaByMatch[index] = { note:data.note || '', publicFixture:data.publicFixture || null, playersLoaded:data.playersLoaded || 0 };
      if (data.publicFixture && !m.publicFixture) m.publicFixture = data.publicFixture;
      playerMarketLoadStatus[index] = playerMarketsByMatch[index].length
        ? `${playerMarketsByMatch[index].length} mercado(s) de jogadores carregados.`
        : (data.note || 'A API respondeu, mas não trouxe amostra suficiente de jogadores para esta partida.');
      if (data.apiFixture) m.apiFixture = data.apiFixture;
      renderMarketDetail(index); populateBuilderMarkets(); populateManualMarkets();
      return playerMarketsByMatch[index];
    } catch (e) {
      playerMarketLoadStatus[index] = e.message || 'Mercados de jogadores indisponíveis nesta partida.';
      const hint = $('#builderMarketHint');
      if (activeBuilderMarketCategory === 'jogadores' && hint) hint.textContent = playerMarketLoadStatus[index];
      return [];
    } finally {
      playerMarketLoading.delete(index);
      if (btn?.isConnected) { btn.disabled=false; btn.textContent = btn.classList?.contains('builder-market-tab') ? 'Jogadores' : 'Tentar novamente'; }
    }
  })();
  playerMarketLoading.set(index, job);
  return job;
}
...
const BUILDER_MARKET_CATEGORIES = [
  ['popular','Popular'],
  ['resultado','Resultado'],
...
function builderMarketText(value='') {
  return String(value || '').normalize('NFD').replace(/[\u0300-\u036f]/g,'').toLowerCase();
}
function builderMarketKinds(m) {
  const market = builderMarketText(m?.market);
  const selection = builderMarketText(m?.selection);
  const key = builderMarketText(m?.key);
  const text = `${market} ${selection} ${key}`;
  const kinds = new Set();

  // Um mercado pode pertencer a mais de uma aba. Ex.: chute de jogador aparece
  // em Jogadores e também em Chutes. Assim categorizar nunca faz um mercado sumir.
  const isPlayer = Boolean(m?.player) || market.includes('jogador') || market.includes('goleiro') || key.startsWith('player_');
  if (isPlayer) kinds.add('jogadores');

  if (market === '1x2' || market.includes('dupla chance') || market.includes('resultado') || market.includes('intervalo/final') || market.includes('handicap') || ['home','draw','away'].includes(key) || key.startsWith('double_chance_') || key.startsWith('htft_')) kinds.add('resultado');

  const isTotals = market.startsWith('total') || market.includes('/total') || market.includes('mais/menos') || key === 'over25' || key === 'under25' || key.startsWith('over_') || key.startsWith('under_') || key.includes('_over_') || key.includes('_under_') || /^(mais|menos) de [0-9]/.test(selection);
  if (isTotals) kinds.add('mais-menos');

  if (market.includes('gol') || market.includes('ambas') || market === 'gols do time' || market === 'gol do jogador' || market.includes('marcador') || market.includes('assist') || key.includes('btts') || key.includes('team_goals') || key.includes('_goal') || key.includes('first_scorer')) kinds.add('gols');
  if (market.includes('escante')) kinds.add('escanteios');
  if (market.includes('cart')) kinds.add('cartoes');
  if (market.includes('chute')) kinds.add('chutes');
  if (market.includes('falta')) kinds.add('faltas');
  if (market.includes('defesa') || market.includes('goleiro')) kinds.add('defesas');

  // Segurança: qualquer mercado novo que o modelo ganhar no futuro continua acessível.
  if (!kinds.size) kinds.add('outros');
  return kinds;
}
function isPopularBuilderMarket(m) {
  const kinds = builderMarketKinds(m);
  if (kinds.has('jogadores')) return false;
  if (kinds.has('resultado') || kinds.has('mais-menos') || kinds.has('gols')) return true;
  if (kinds.has('escanteios') && Number(m?.probability || 0) >= .68) return true;
  return false;
}
function builderMarketsForCategory(markets, category) {
  const indexed = (markets || []).map((market,index)=>({ market,index }));
  if (category === 'popular') {
    return indexed.filter(({market})=>isPopularBuilderMarket(market))
      .sort((a,b)=>Number(b.market?.probability||0)-Number(a.market?.probability||0))
      .slice(0,18);
  }
  return indexed.filter(({market})=>builderMarketKinds(market).has(category));
}
function playerPositionLabel(position='') {
  const p = String(position || '').toUpperCase();
  if (p.startsWith('G')) return 'GOL';
  if (p.startsWith('D')) return 'DEF';
  if (p.startsWith('M')) return 'MEI';
  if (p.startsWith('F')) return 'ATA';
  return p.slice(0,3) || 'JOG';
}
function playerInitials(name='') {
  return String(name || '?').trim().split(/\s+/).slice(0,2).map(x=>x[0]||'').join('').toUpperCase() || 'J';
}
function playerAvatar(m, cls='player-row-avatar') {
  if (m?.photo) return `<img src="${safe(m.photo)}" alt="" loading="lazy" referrerpolicy="no-referrer" />`;
  return `<span class="${safe(cls)}">${safe(playerInitials(m?.player))}</span>`;
}
function teamLogoForBuilder(match, teamName='') {
  const fixture = match?.publicFixture || match?.favoriteFixture || null;
  if (!fixture) return '';
  const wanted = builderMarketText(teamName);
  if (builderMarketText(fixture.home?.name) === wanted) return fixture.home?.logo || '';
  if (builderMarketText(fixture.away?.name) === wanted) return fixture.away?.logo || '';
  if (builderMarketText(match?.homeTeam) === wanted) return fixture.home?.logo || '';
  if (builderMarketText(match?.awayTeam) === wanted) return fixture.away?.logo || '';
  return '';
}
function teamLogoHtml(match, teamName='') {
  const logo = teamLogoForBuilder(match, teamName);
  return logo ? `<img src="${safe(logo)}" alt="" loading="lazy" referrerpolicy="no-referrer" />` : `<span class="team-placeholder">${safe(playerInitials(teamName))}</span>`;
}
function playerMarketTitle(name='') {
  return ({
    'Chutes no gol do jogador':'Chutes no gol',
    'Chutes do jogador':'Chutes',
    'Defesas do goleiro':'Defesas do goleiro',
    'Assistências do jogador':'Assistências',
    'Passes do jogador':'Passes',
    'Desarmes do jogador':'Desarmes',
    'Impedimentos do jogador':'Impedimentos',
    'Faltas do jogador':'Faltas cometidas',
    'Faltas sofridas':'Faltas sofridas',
    'Cartão do jogador':'Cartões',
    'Gol do jogador':'Marcador',
    'Primeiro marcador':'Primeiro marcador'
  })[name] || name;
}
function playerLineLabel(m) {
  const market = String(m?.market || '');
  if (market === 'Primeiro marcador') return 'Primeiro';
  if (market === 'Cartão do jogador') return 'Cartão';
  if (market === 'Gol do jogador' && Number(m?.target) === 1) return 'Qualquer momento';
  if (market === 'Gol do jogador' && Number(m?.target) === 3) return 'Hat-Trick';
  const t = Number(m?.target);
  return Number.isFinite(t) && t > 0 ? `${t}+` : 'Selecionar';
}
function uniquePlayerPreview(markets, teamName) {
  const seen = new Set(), rows = [];
  for (const m of markets || []) {
    if (!m?.player || builderMarketText(m.team) !== builderMarketText(teamName)) continue;
    const key = builderMarketText(m.player);
    if (seen.has(key)) continue;
    seen.add(key);
    rows.push(m);
  }
  const score = row => Number(row.starts || 0) * 5 + Number(row.appearances || 0) + Number(row.expectedMinutes || 0) / 90;
  rows.sort((a,b)=>score(b)-score(a));
  const quotas = [['G',1],['D',4],['M',3],['F',3]];
  const selected = [], used = new Set();
  for (const [pos,limit] of quotas) {
    for (const row of rows.filter(x=>String(x.position||'').toUpperCase().startsWith(pos)).slice(0,limit)) {
      selected.push(row); used.add(builderMarketText(row.player));
    }
  }
  for (const row of rows) {
    if (selected.length >= 11) break;
    if (!used.has(builderMarketText(row.player))) { selected.push(row); used.add(builderMarketText(row.player)); }
  }
  return selected.slice(0,11);
}
function lineupLane(position='') {
  const p=String(position||'').toUpperCase();
...
function renderPitchTeam(markets, team, side, matchIndex) {
  const players=uniquePlayerPreview(markets,team);
  const grouped={gk:[],def:[],mid:[],att:[]};
  players.forEach(p=>(grouped[lineupLane(p.position)]||grouped.att).push(p));
  const focused=builderMarketText(activePlayerFocusByMatch[matchIndex]||'');
  const lane=(name,rows)=>`<div class="pitch-lane pitch-${safe(name)}">${rows.map(p=>{const key=builderMarketText(p.player);return `<button type="button" class="pitch-player${focused===key?' is-focused':''}" data-player-focus="${safe(key)}" aria-label="Ver mercados de ${safe(p.player)}">${playerAvatar(p,'pitch-avatar')}<b>${safe(String(p.player||'').split(/\s+/).slice(-1)[0]||p.player)}</b></button>`}).join('')}</div>`;
  return `<div class="pitch-team pitch-team-${safe(side)}">${lane('gk',grouped.gk)}${lane('def',grouped.def)}${lane('mid',grouped.mid)}${lane('att',grouped.att)}</div>`;
}
function renderPlayerLineupPreview(match, markets, matchIndex) {
  const home=match?.homeTeam, away=match?.awayTeam;
  return `<section class="probable-lineup"><div class="probable-lineup-head"><strong>Prováveis escalações</strong><small>referência por posição e uso recente</small></div><div class="lineup-teams"><span>${teamLogoHtml(match,home)}<b>${safe(home||'Casa')}</b></span><span>${teamLogoHtml(match,away)}<b>${safe(away||'Fora')}</b></span></div><div class="lineup-pitch">${renderPitchTeam(markets,home,'home',matchIndex)}<span class="pitch-center-line"></span>${renderPitchTeam(markets,away,'away',matchIndex)}</div><small class="lineup-note">A disposição é uma prévia do Bilhete Plus; a escalação oficial pode mudar antes da partida.</small></section>`;
}
function playerCardPriority(m) {
  const order=['Gol do jogador','Primeiro marcador','Chutes do jogador','Chutes no gol do jogador','Faltas sofridas','Faltas do jogador','Cartão do jogador','Defesas do goleiro','Assistências do jogador','Passes do jogador','Desarmes do jogador','Impedimentos do jogador'];
  const i=order.indexOf(String(m.market||''));
  const target=Number(m.target||0);
  return (i<0?99:i)*100 + (target||0);
}
function playerOptionLabel(m) {
  const base=playerMarketTitle(m.market);
  const line=playerLineLabel(m);
  if (m.market==='Gol do jogador') return line==='Qualquer momento' ? 'Marcar a qualquer momento' : `${line} gol`;
  if (m.market==='Primeiro marcador') return 'Primeiro marcador';
  if (m.market==='Cartão do jogador') return 'Receber cartão';
  return `${line} ${String(base||'').toLowerCase()}`.replace(/^1\+ defesas do goleiro$/,'1+ defesa');
}
function renderPlayerOption(m, matchIndex) {
  const signature=`${matchIndex}|${m.key}|${m.selection}`;
  const selected=customTicket.legs.some(l=>l.signature===signature);
  const real=m.oddType==='real'&&m.odd;
  const price=real?m.odd:(m.displayOdd||m.fairOdd||1/Math.max(.001,m.probability));
  return `<button type="button" class="player-card-market${selected?' is-selected':''}" data-player-market-index="${m._allIndex}"><span>${safe(playerOptionLabel(m))}</span><b>${pct(Number(m.probability||0))}</b><small>Justa ${money(m.fairOdd||1/Math.max(.001,m.probability))}</small><strong>${money(price)}</strong></button>`;
}
function renderPlayerMarketBoard(matchIndex, indexedRows=[]) {
  const board=$('#playerMarketBoard'); if(!board) return;
  const match=dashboard?.matches?.[matchIndex];
  const rows=indexedRows.map(x=>({...x.market,_allIndex:x.index}));
  const status=playerMarketLoadStatus[matchIndex]||'';
  if(!match){board.innerHTML='';return;}
  if(!rows.length){board.innerHTML=`<div class="player-market-empty ${playerMarketLoading.has(matchIndex)?'player-market-loading':''}">${safe(status||'Carregando mercados de jogadores desta partida…')}</div>`;return;}
  const byPlayer=new Map();
  for(const row of rows){
    const key=`${builderMarketText(row.team)}|${builderMarketText(row.player||row.selection)}`;
    if(!byPlayer.has(key))byPlayer.set(key,[]);
    byPlayer.get(key).push(row);
  }
  const allPlayers=[...byPlayer.values()].sort((a,b)=>{
    const ta=builderMarketText(a[0]?.team),tb=builderMarketText(b[0]?.team);
    const home=builderMarketText(match.homeTeam);
    if(ta!==tb)return ta===home?-1:tb===home?1:ta.localeCompare(tb);
    return Math.max(...b.map(x=>Number(x.probability||0)))-Math.max(...a.map(x=>Number(x.probability||0)));
  });
  const focus=builderMarketText(activePlayerFocusByMatch[matchIndex]||'');
  const players=focus ? allPlayers.filter(options=>builderMarketText(options[0]?.player)===focus) : allPlayers;
  if(focus && !players.length) delete activePlayerFocusByMatch[matchIndex];
  const focusedOptions=players[0]||null;
  const focusedName=focus && focusedOptions ? focusedOptions[0]?.player : '';
  const meta=playerMarketMetaByMatch[matchIndex]||{};
  const focusBar=focusedName ? `<div class="player-focus-bar"><div><small>Mercados do jogador</small><strong>${safe(focusedName)}</strong></div><button type="button" class="secondary clear-player-focus">Ver todos os jogadores</button></div>` : '';
  board.innerHTML=`<section class="player-market-intro player-market-page"><div class="player-market-intro-head"><div><span class="eyebrow">JOGADORES</span><h4>${safe(match.homeTeam)} x ${safe(match.awayTeam)}</h4></div><span class="player-market-count">${allPlayers.length} jogadores</span></div>${renderPlayerLineupPreview(match,rows,matchIndex)}${focusBar}<div class="micro">${safe(meta.note||'Toque em um jogador na escalação para ver somente os mercados dele. Quando não houver odd real, exibimos a odd justa/estimada do modelo.')}</div></section><div class="player-card-list">${players.map(options=>{
    const goalkeeper=String(options[0]?.position||'').toUpperCase().startsWith('G');
    const sorted=[...options].sort((a,b)=>{
      if(goalkeeper){const ag=a.market==='Defesas do goleiro'?-100:0,bg=b.market==='Defesas do goleiro'?-100:0;if(ag!==bg)return ag-bg;}
      return playerCardPriority(a)-playerCardPriority(b)||Number(b.probability||0)-Number(a.probability||0);
    });
    const base=sorted[0], primary=sorted.slice(0,5), extra=sorted.slice(5);
    return `<article class="player-prop-card${focus?' focused-player-card':''}"><div class="player-prop-head"><div class="player-prop-person">${playerAvatar(base,'player-prop-avatar')}<div><strong>${safe(base.player||base.selection)}</strong><small>${safe(base.team||'')} • ${safe(playerPositionLabel(base.position))}</small></div></div>${!focus?`<button type="button" class="player-focus-button" data-player-focus-card="${safe(builderMarketText(base.player))}">Ver jogador</button>`:''}</div><div class="player-prop-markets">${primary.map(m=>renderPlayerOption(m,matchIndex)).join('')}</div>${extra.length?`<details class="player-more-inline"><summary>Mais mercados (${extra.length})</summary><div class="player-more-grid">${extra.map(m=>renderPlayerOption(m,matchIndex)).join('')}</div></details>`:''}</article>`;
  }).join('')}</div>`;
  board.querySelectorAll('.pitch-player[data-player-focus], .player-focus-button[data-player-focus-card]').forEach(btn=>btn.addEventListener('click',()=>{
    const key=btn.dataset.playerFocus||btn.dataset.playerFocusCard||'';
    if(!key)return;
    activePlayerFocusByMatch[matchIndex]=key;
    renderPlayerMarketBoard(matchIndex,builderMarketsForCategory(allMarketsForMatch(matchIndex),'jogadores'));
    requestAnimationFrame(()=>board.querySelector('.player-focus-bar')?.scrollIntoView({behavior:'smooth',block:'nearest'}));
  }));
  board.querySelector('.clear-player-focus')?.addEventListener('click',()=>{
    delete activePlayerFocusByMatch[matchIndex];
    renderPlayerMarketBoard(matchIndex,builderMarketsForCategory(allMarketsForMatch(matchIndex),'jogadores'));
  });
  board.querySelectorAll('.player-card-market').forEach(btn=>btn.addEventListener('click',()=>{
    const market=allMarketsForMatch(matchIndex)[Number(btn.dataset.playerMarketIndex)];
    if(market){addCustomCandidate(matchIndex,market);populateBuilderMarkets();}
  }));
}

function enableBuilderTabsMouseDrag(holder) {
  if (!holder || holder.dataset.mouseDragReady === '1') return;
  holder.dataset.mouseDragReady = '1';
  let dragging = false, moved = false, startX = 0, startScroll = 0;
  holder.addEventListener('mousedown', e => {
    if (e.button !== 0) return;
    dragging = true; moved = false; startX = e.clientX; startScroll = holder.scrollLeft;
  ...
  window.addEventListener('mousemove', e => {
    ...
    if (!moved) return;
    holder.scrollLeft = startScroll - dx;
    e.preventDefault();
  });
  window.addEventListener('mouseup', () => {
    if (!dragging) return;
    dragging = false; holder.classList.remove('is-dragging');
    if (moved) holder._suppressBuilderClickUntil = Date.now() + 180;
  });
  holder.addEventListener('click', e => {
    if (Date.now() < Number(holder._suppressBuilderClickUntil || 0)) {
      e.preventDefault(); e.stopPropagation();
    }
  }, true);
}
function renderBuilderMarketTabs(markets) {
  const holder = $('#builderMarketTabs'); if (!holder) return;
  const matchIndex = Number($('#builderMatch').value || 0);
  const hasMatch = Boolean(dashboard?.matches?.[matchIndex]);
  // Jogadores fica sempre visível quando existe uma partida selecionada. Durante
  // limpeza/troca de filtros não tentamos carregar jogadores sem jogo — isso evita
  // um ciclo de microtarefas que podia congelar a tela em "Carregando mercados".
  const available = BUILDER_MARKET_CATEGORIES.filter(([key]) => hasMatch && (key === 'jogadores' || builderMarketsForCategory(markets,key).length));
  if (!available.some(([key])=>key===activeBuilderMarketCategory)) activeBuilderMarketCategory = available[0]?.[0] || 'popular';
  holder.innerHTML = available.map(([key,label])=>`<button type="button" class="builder-market-tab${key===activeBuilderMarketCategory?' active':''}" data-builder-category="${key}">${safe(label)}</button>`).join('');
  enableBuilderTabsMouseDrag(holder);
  holder.querySelectorAll('.builder-market-tab').forEach(btn=>btn.addEventListener('click',async()=>{
    activeBuilderMarketCategory = btn.dataset.builderCategory || 'popular';
    const needsPlayers = activeBuilderMarketCategory === 'jogadores' && !builderMarketsForCategory(allMarketsForMatch(matchIndex),'jogadores').length;
    populateBuilderMarkets();
    if (needsPlayers) {
      const currentTab = $('#builderMarketTabs .builder-market-tab[data-builder-category="jogadores"]');
      if (currentTab) { currentTab.disabled = true; currentTab.textContent = 'Carregando…'; }
      await loadPlayerMarkets(matchIndex, currentTab || btn);
      activeBuilderMarketCategory = 'jogadores';
      populateBuilderMarkets();
    }
  }));
}
function populateBuilderMarkets(options={}) {
  if (options.resetCategory) activeBuilderMarketCategory = 'popular';
  const index = Number($('#builderMatch').value || 0);
  const match = dashboard?.matches?.[index];
  if (!match) {
    const holder = $('#builderMarketTabs'); if (holder) holder.innerHTML = '';
    const board = $('#playerMarketBoard'); if (board) { board.hidden = true; board.innerHTML = ''; }
    const select = $('#builderMarket');
    if (select) { select.innerHTML = '<option value="">Analise os jogos primeiro</option>'; select.disabled = true; }
    const add = $('#builderAdd'); if (add) add.disabled = true;
    const hint = $('#builderMarketHint'); if (hint) hint.textContent = 'Analise os jogos para escolher os mercados.';
    return;
  }
  const markets = allMarketsForMatch(index);
  renderBuilderMarketTabs(markets);
  let filtered = builderMarketsForCategory(markets, activeBuilderMarketCategory);
  if (!filtered.length && markets.length && activeBuilderMarketCategory !== 'jogadores') {
    activeBuilderMarketCategory = 'popular';
    renderBuilderMarketTabs(markets);
    filtered = builderMarketsForCategory(markets, activeBuilderMarketCategory);
  }
  const select = $('#builderMarket');
  const playerMode = activeBuilderMarketCategory === 'jogadores';
  const board = $('#playerMarketBoard');
  const selectionLabel = $('.builder-selection-label');
  if (board) board.hidden = !playerMode;
  if (selectionLabel) selectionLabel.hidden = playerMode;
  select.hidden = playerMode;
  const addButton = $('#builderAdd');
  if (addButton) addButton.hidden = playerMode;
  select.innerHTML = filtered.length
    ? filtered.map(({market,index})=>`<option value="${index}">${safe(market.selection)} • ${pct(market.probability)} • justa ${money(market.fairOdd || 1/Math.max(.001,market.probability))}</option>`).join('')
    : '<option value="">Nenhum mercado nesta categoria</option>';
  select.disabled = !filtered.length;
  if (addButton) addButton.disabled = !filtered.length;
  if (playerMode) renderPlayerMarketBoard(index, filtered);
  const hint = $('#builderMarketHint');
  if (hint) hint.textContent = filtered.length
    ? `${filtered.length} mercado(s) nesta categoria`
    : activeBuilderMarketCategory === 'jogadores'
      ? (playerMarketLoadStatus[index] || 'Carregando mercados de jogadores desta partida…')
      : 'Escolha outra categoria de mercado.';
  // Se Jogadores já estiver selecionado (por exemplo, após a interface ser re-renderizada),
  // não exigimos um segundo clique. Uma única busca é disparada e deduplicada por partida.
  if (dashboard?.matches?.[index] && activeBuilderMarketCategory === 'jogadores' && !filtered.length && !playerMarketLoading.has(index) && !playerMarketLoadStatus[index]) {
    const tab = $('#builderMarketTabs .builder-market-tab[data-builder-category="jogadores"]');
    queueMicrotask(async()=>{
      if (tab?.isConnected) { tab.disabled=true; tab.textContent='Carregando…'; }
      await loadPlayerMarkets(index, tab);
      if (activeBuilderMarketCategory === 'jogadores') populateBuilderMarkets();
    });
  }
}
...
function addCustomCandidate(matchIndex, market) {
  ...
  renderCustomTicket(); pulseFloatingTicket();
}
...
function renderCustomTicket() {
  const legs=customTicket.legs||[];
  const countText=`${legs.length} seleç${legs.length===1?'ão':'ões'}`;
  if($('#customCount')) $('#customCount').textContent=countText;
  if($('#floatingTicketCount')) $('#floatingTicketCount').textContent=String(legs.length);
  if($('#floatingTicketButton')) $('#floatingTicketButton').classList.toggle('has-legs',legs.length>0);
...
function openFloatingTicket() {
  const drawer=$('#floatingTicketDrawer'), backdrop=$('#floatingTicketBackdrop'), button=$('#floatingTicketButton');
...
function pulseFloatingTicket() {
  const button=$('#floatingTicketButton'); if(!button)return;
  button.classList.remove('ticket-added'); void button.offsetWidth; button.classList.add('ticket-added');
  setTimeout(()=>button.classList.remove('ticket-added'),650);
}
...
function saveCustomTicket() {
  ...
  closeFloatingTicket(); switchTab('live'); renderSavedTickets();
  return item;
}
...
function automaticTimeBand() {
  return document.querySelector('input[name="ticketTimeBand"]:checked')?.value || 'day-all';
}

function automaticTimeBandLabel() {
  return ({ 'day-all':'Dia todo', morning:'Manhã', afternoon:'Tarde', night:'Noite' })[automaticTimeBand()] || 'Dia todo';
}
...
async function fetchDashboard(params) {
  ...
  const requestOnce = async (url, timeoutMs = 65000) => {
    const controller = new AbortController();
    const timer = setTimeout(() => controller.abort(), timeoutMs);
    try { return await fetch(url, { signal:controller.signal, cache:'no-store' }); }
    catch (error) {
      if (error?.name === 'AbortError') throw new Error('O servidor não terminou de acordar a tempo. Toque novamente em Analisar jogos.');
      throw error;
    } finally { clearTimeout(timer); }
  };

  const requestResilient = async (url) => {
    // Dispara o aquecimento sem bloquear. Em servidor já acordado, /api/dashboard
    // responde em poucos segundos; em cold start, a requisição permanece viva.
    wakeBackend().catch(()=>{});
    let r;
    try {
      r = await requestOnce(url, 65000);
    } catch (firstError) {
      // Em alguns cold starts o proxy do Render devolve erro transitório antes do
      // processo Node ficar pronto. Aguarda o health e repete uma única vez.
      const woke = await wakeBackend().catch(()=>false);
      if (!woke) throw firstError;
      r = await requestOnce(url, 22000);
    }
    if ([502,503,504].includes(r.status)) {
      const woke = await wakeBackend().catch(()=>false);
      if (woke) r = await requestOnce(url, 22000);
    }
    return r;
  };

  let r = await requestResilient(`/api/dashboard?${q}`);
  let data = await r.json().catch(() => ({}));
  if (r.ok) { backendLastReadyAt = Date.now(); return data; }

  const rawError = String(data.error || `Falha ao carregar (${r.status})`);
  ...
  if (quota && q.get('forcePublic') !== '1') {
    q.set('forcePublic','1');
    r = await requestResilient(`/api/dashboard?${q}`);
...
async function loadAutomaticTickets({ reuseDashboard = false } = {}) {
  ...
  const includeWomen = $('#ticketIncludeWomen')?.checked === true;
  $('#tickets').innerHTML = `<section class="card loading">Montando sugestões de ${safe(automaticLeagueLabel())}…</section>`;
  $('#ticketFilterSummary').innerHTML = `<span class="pill">${safe(automaticLeagueLabel())}</span><span class="pill">${safe(automaticTimeBandLabel())}</span><span class="pill">${includeWomen || league === 'womens-football' ? 'Feminino incluído' : 'Somente masculino'}</span><span class="pill">carregando…</span>`;
  $('#generateAutomaticTickets').disabled = true;
  try {
    let data = null;
    const band = automaticTimeBand();
    const canReuse = reuseDashboard && dashboard && dashboard.date === date && dashboard.leagueKey === league && (dashboard.timeBand || 'day-all') === band && Boolean(dashboard.includeWomen) === includeWomen && $('#ticketAutoPlayers').checked === $('#autoPlayers').checked;
    if (canReuse) {
      data = dashboard;
    } else {
      // Primeiro monta os bilhetes sem esperar a API de jogadores. Assim a tela responde rápido.
      const q = new URLSearchParams({ date, league, timeBand:band, includeWomen:includeWomen ? '1' : '0' });
      data = await fetchDashboard(q);
    }
    ticketDashboard = data;
    // Se esse carregamento trouxe mercados mais ricos (principalmente jogadores),
    // disponibilize-os imediatamente no Montar meu bilhete da mesma data/liga.
    if (dashboard && dashboard.date === data.date && dashboard.leagueKey === data.leagueKey) {
      populateBuilderMarkets();
      populateManualMarkets();
      if ($('#marketDetail')) renderMarketDetail(Number($('#marketMatch').value || 0));
    }
    ...
    $('#ticketFilterSummary').innerHTML = `<span class="pill">${safe(automaticLeagueLabel())}</span><span class="pill">${safe(automaticTimeBandLabel())}</span><span class="pill">${includeWomen || league === 'womens-football' ? 'Feminino incluído' : 'Somente masculino'}</span><span class="pill">${data.count} jogo(s) pré-jogo</span>${data.sourceStatus?.publicFixtures&&data.sourceStatus.publicFixtures!=='não usado'?`<span class="pill">Fonte: ${safe(data.sourceStatus.publicFixtures)}</span>`:''}${data.sourceStatus?.jogadores?`<span class="pill">Jogadores: ${safe(data.sourceStatus.jogadores)}</span>`:''}`;
    wireAutomaticTickets();

    // Jogadores entram em segundo plano. Nunca bloqueiam a geração principal.
    if ($('#ticketAutoPlayers').checked && data.count > 0) {
      const stamp = `${date}|${league}|${band}|${includeWomen ? 1 : 0}`;
      setTimeout(async () => {
        try {
          const qPlayers = new URLSearchParams({ date, league, includePlayers:'1', timeBand:band, includeWomen:includeWomen ? '1' : '0' });
          const richer = await fetchDashboard(qPlayers);
          if (`${$('#ticketDate').value}|${$('#ticketLeague').value}|${automaticTimeBand()}|${$('#ticketIncludeWomen')?.checked ? 1 : 0}` !== stamp) return;
          ticketDashboard = richer;
          $('#tickets').innerHTML = richer.count > 0
            ? renderTickets(richer.tickets || [], richer.ticketGroups)
            : $('#tickets').innerHTML;
          $('#ticketFilterSummary').innerHTML = `<span class="pill">${safe(automaticLeagueLabel())}</span><span class="pill">${safe(automaticTimeBandLabel())}</span><span class="pill">${includeWomen || league === 'womens-football' ? 'Feminino incluído' : 'Somente masculino'}</span><span class="pill">${richer.count} jogo(s) pré-jogo</span>${richer.sourceStatus?.publicFixtures&&richer.sourceStatus.publicFixtures!=='não usado'?`<span class="pill">Fonte: ${safe(richer.sourceStatus.publicFixtures)}</span>`:''}${richer.sourceStatus?.jogadores?`<span class="pill">Jogadores: ${safe(richer.sourceStatus.jogadores)}</span>`:''}`;
          wireAutomaticTickets();
          if (dashboard && dashboard.date === richer.date && dashboard.leagueKey === richer.leagueKey) {
            populateBuilderMarkets(); populateManualMarkets();
          }
        } catch {
          // Silencioso por design: bilhetes normais permanecem utilizáveis.
        }
      }, 250);
    }
  } catch (e) {
    ...
    $('#ticketFilterSummary').innerHTML = `<span class="pill">${safe(automaticLeagueLabel())}</span><span class="pill">${safe(automaticTimeBandLabel())}</span><span class="pill">0 jogo(s)</span><span class="pill">dados não atualizados</span>`;
  } finally {
...
async function load() {
  clearGameState();
  $('#load').disabled = true;
  $('#load').textContent = 'Analisando…';
  $('#matches').innerHTML=`<div class="card loading">Carregando jogos e mercados…</div>`;
  const slowHint = setTimeout(() => {
    if ($('#load').disabled) $('#matches').innerHTML=`<div class="card loading"><strong>Acordando o servidor…</strong><span>Se o Render estava em repouso, a primeira abertura pode demorar um pouco. A análise continuará automaticamente.</span></div>`;
  }, 6500);
  try {
    // fast=1: a aba Jogos recebe partidas/mercados sem executar o beam-search dos
    // bilhetes automáticos. Esses bilhetes são calculados só quando o usuário toca
    // em "Gerar sugestões", removendo trabalho pesado do botão Analisar jogos.
    const q=new URLSearchParams({date:$('#date').value,league:$('#league').value,fast:'1'});
    // A abertura da aba Jogos precisa ser imediata. Mercados de jogadores continuam
    // disponíveis sob demanda e nos Automáticos, mas não bloqueiam o primeiro carregamento.
    const data=await fetchDashboard(q);
    dashboard=data;playerMarketsByMatch={};playerMarketLoadStatus={};playerMarketLoading=new Map();
    ...
    if (!data.ticketsDeferred && !ticketDashboard && $('#ticketDate').value === $('#date').value && $('#ticketLeague').value === $('#league').value) {
      ticketDashboard = data;
      $('#tickets').innerHTML=renderTickets(data.tickets || [], data.ticketGroups || {});
      $('#ticketFilterSummary').innerHTML=`<span class="pill">${safe(automaticLeagueLabel())}</span><span class="pill">${safe(automaticTimeBandLabel())}</span><span class="pill">${data.count} jogo(s) pré-jogo</span>${data.sourceStatus?.jogadores?`<span class="pill">Jogadores: ${safe(data.sourceStatus.jogadores)}</span>`:''}`;
      wireAutomaticTickets();
    }
    ...
    $('#load').disabled = false;
    $('#load').textContent = 'Analisar jogos';
  }
}
...
$$('.filter-chip').forEach(b=>b.addEventListener('click',async()=>{
  activeMarketFilter=b.dataset.filter;
  $$('.filter-chip').forEach(x=>x.classList.toggle('active',x===b));
  const index=Number($('#marketMatch').value||0);
  renderMarketDetail(index);
  // Na aba Mercados, tocar em Jogadores agora é suficiente: a busca começa
  // automaticamente. Antes o usuário ainda precisava descobrir/clicar em um
  // segundo botão e, após falha, via apenas "Nenhum mercado desta categoria".
  if (activeMarketFilter === 'jogadores' && dashboard?.matches?.[index]) {
    const existing=(playerMarketsByMatch[index]||[]).length;
    if (!existing && !playerMarketLoading.has(index)) {
      const retry=$('.load-player-market');
      await loadPlayerMarkets(index,retry);
      if (activeMarketFilter === 'jogadores') renderMarketDetail(index);
    }
  }
}));
$('#marketMatch').addEventListener('change',()=>renderMarketDetail(Number($('#marketMatch').value)));
$('#builderMatch').addEventListener('change',()=>populateBuilderMarkets({resetCategory:true}));
...
$('#ticketIncludeWomen')?.addEventListener('change',()=>{ ticketDashboard=null; $('#ticketFilterSummary').innerHTML=''; });
...
function invalidateAutomaticTickets() {
  ticketDashboard = null;
  $('#ticketFilterSummary').innerHTML=`<span class="pill">${safe(automaticLeagueLabel())}</span><span class="pill">${safe(automaticTimeBandLabel())}</span><span class="pill">toque em Gerar sugestões</span>`;
  $('#tickets').innerHTML='<section class="card empty">Filtro alterado. Toque em <strong>Gerar sugestões</strong> para montar novos bilhetes somente com esta competição e faixa de horário.</section>';
}
...
document.querySelectorAll('input[name="ticketTimeBand"]').forEach(input=>input.addEventListener('change',invalidateAutomaticTickets));
...
$('#floatingTicketButton').addEventListener('click',openFloatingTicket);
$('#closeFloatingTicket').addEventListener('click',closeFloatingTicket);
$('#floatingTicketBackdrop').addEventListener('click',closeFloatingTicket);
document.addEventListener('keydown',e=>{if(e.key==='Escape'&&$('#floatingTicketDrawer')?.classList.contains('open'))closeFloatingTicket();});

updateBuilder();renderLiveTicket();renderCustomTicket();renderSavedTickets();renderComparisonHistory();
// A interface aparece primeiro. Consultas externas só começam quando o usuário pede.
$('#health').textContent = 'pronto';
$('#matches').innerHTML = `<div class="card empty">Escolha a data/competição e toque em <strong>Analisar jogos</strong>.</div>`;
$('#tickets').innerHTML = `<section class="card empty">Escolha o filtro e toque em <strong>Gerar sugestões</strong>.</section>`;
setTimeout(() => health().catch(()=>{}), 50);
